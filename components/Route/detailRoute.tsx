...
import DetailRoutefication from "./VehicleTable/vehiclePopup";
...
const DetailRoute: React.FC<DetailRouteProps> = ({ onClose, reloadData, dataInitial }) => {
    const notificationRef = useRef<HTMLDivElement>(null);
    const [isVisible, setIsVisible] = useState(true);
    const data = dataInitial;
    const routesOperation = new RoutesOperation()
    const [departureTime, setDepartureTime] = useState<string>(data.departure_time);
    const adminOperation = new AdministrativeOperation();
    const [vehicleID, setVehicleID] = useState<string>(data.vehicle_id);
    const [provinces, setProvinces] = useState([]);
    const [selectedProvince, setSelectedProvince] = useState<null | string>(data.source);
    const [selectedProvince2, setSelectedProvince2] = useState<null | string>(data.destination);
    ...
    const handleConfirm = async () => {
        const routeid = { id: dataInitial.id }
        const isDataChanged =
            vehicleID !== dataInitial.vehicle_id ||
            selectedProvince !== dataInitial.source ||
            selectedProvince2 !== dataInitial.destination ||
            departureTime !== dataInitial.departure_time;
        ...
        if (isDataChanged) {
            ...
            const result = await routesOperation.update(routeid, info2);
    ...
    return (
        ...
        >
            {openModal && <DetailRoutefication onClose={() => setModalOpen(false)} setVehicleID={(vehicleid) => setVehicleID(vehicleid)} />}
            <motion.div
                ref={notificationRef}
