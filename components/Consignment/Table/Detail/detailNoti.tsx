...
const DetailNotification: React.FC<DetailNotificationProps> = ({
  ...
  const handleDeleteSelectedOrders = async () => {
    try {
      if (selectedOrders.length > 0) {
        ...
        const result = await shipmentsOperation.deleteOrderFromShipment(
          condition,
          info
        );
        if (result.error) {
  ...
  const addOrders = async (newOrders) => {
    try {
      ...
      const result = await shipmentsOperation.addOrdersToShipment(
        condition,
        info
      );
      if (result.error) {
    ...
    >
      ...
      {openConfirm && (
        <SubmitPopup
          ...
          message={message}
          submit={handleDeleteSelectedOrders}
        />
      )}
      {modalIsOpen && <AddOrders onClose={closeModal} addOrders={addOrders} />}
      {modalIsOpen2 && (
        <TimelineNoti onClose={closeModal2} dataInitial={dataInitial} />
      )}
      <motion.div
        ref={notificationRef}
      ...
      >
        <div className="relative items-center justify-center flex-col flex h-10 w-full border-b-2 border-[#545e7b]">
          <div className="font-bold text-lg sm:text-2xl pb-2 dark:text-white w-full text-center">
            <FormattedMessage id="Consignment.Info.Title" />
          </div>
          <Button
        ...
        <div className="h-screen_3/5 overflow-y-scroll border border-[#545e7b] mt-4 no-scrollbar flex flex-col bg-white dark:bg-[#14141a] p-2 rounded-md dark:text-white">
          <div className="flex flex-col">
            <div className="text-center text-lg sm:text-xl font-semibold">
              <FormattedMessage id="Consignment.Info.Info1" />:{" "}
              {data.shipment_id}
            </div>
            <div className="pt-2 pl-2 w-full flex flex-col items-center">
              <div className="flex w-full lg:w-1/2">
                <span className="font-bold mr-2 whitespace-nowrap">
                  + <FormattedMessage id="Consignment.Info.Info2" />:
                </span>
              </div>
              <div className="flex w-full lg:w-1/2">
                <span className="font-bold mr-2 whitespace-nowrap">
                  + <FormattedMessage id="Consignment.Info.Info3" />:
                </span>
                <span>{data.mass}</span>
              </div>
              <div className="flex w-full lg:w-1/2">
                <span className="font-bold mr-2 whitespace-nowrap ư">
                  + <FormattedMessage id="Consignment.Info.Info5" />:
                </span>
                ...
                switch (data.status) {
                  ...
                  default:
                    statusLabel = intl.formatMessage({
                      id: "Consignment.Status.NotDecomposed",
                    });
                    statusColor = "text-black";
                }

                return (
                  <span className={`${statusColor} font-semibold text-xl`}>
                    {statusLabel}
                  </span>
                );
              })()}
            ...
            <div className="w-full flex">
              {selectedOrders.length > 0 && (
                <Button
                  ...
                  onClick={() => {
                    setMessage(
                      intl.formatMessage({ id: "Consignment.Info.Submit" })
                    );
                    setOpenConfirm(true);
                  }}
                >
                  ...
                  <span className="block">
                    <FormattedMessage id="Consignment.Info.Confirm" />
                  </span>
                </Button>
              )}
              <div className="grow flex justify-end">
                ...
                >
                  ...
                  <span className="block">
                    <FormattedMessage id="Consignment.Info.Add1" />
                  </span>
                </Button>
                <Button
                  className={`h-10 rounded-lg mt-5 mb-1 p-3 border-green-700 hover:bg-green-700 text-green-500
                ...
                >
                  ...
                  <span className="block">
                    <FormattedMessage id="Consignment.Info.Add2" />
                  </span>
                </Button>
              </div>
            </div>
            ...
            <AnimatePresence initial={false}>
              {orders.map((order) => (
                <motion.div
                  ...
                  className={`border border-[#545e7b] rounded-lg p-2 dark:bg-[#1a1b23] flex flex-col ${
                ...
                >
                  ...
                  <div className="pt-2 pl-2 w-full flex flex-col items-center">
                    <div className="flex w-full">
                      <span className="font-bold mr-2 whitespace-nowrap">
                        + <FormattedMessage id="Consignment.Info.Info9" />:
                      </span>
                    ...
                    <div className="flex w-full gap-2">
                      <div>
                        <span className="font-bold mr-2 whitespace-nowrap">
                          + <FormattedMessage id="Consignment.Info.Info10" />:
                        </span>
                        <span>{order.length}</span>
                      </div>
                      <div>
                        <span className="font-bold mr-2 whitespace-nowrap">
                          + <FormattedMessage id="Consignment.Info.Info11" />:
                        </span>
                        <span>{order.width}</span>
                      </div>
                      <div>
                        <span className="font-bold mr-2 whitespace-nowrap">
                          + <FormattedMessage id="Consignment.Info.Info12" />:
                        </span>
                    ...
                    <div className="flex w-full">
                      <span className="font-bold mr-2 whitespace-nowrap">
                        + <FormattedMessage id="Consignment.Info.Info16" />:
                      </span>
                      <span>
                        {order.COD.toLocaleString("vi-VN", {
                          style: "currency",
                          currency: "VND",
                        })}
                      </span>
                  ...
                  <div className="text-center text-lg mt-2">
                    ...
                    {(() => {
                      ...
                      switch (data.status) {
                        case 1:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.DeliveredSuccess",
                          });
                          statusColor = "text-green-500";
                          break;
                        case 2:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.Processing",
                          });
                          statusColor = "text-black";
                          break;
                        case 3:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.Taking",
                          });
                          statusColor = "text-black";
                          break;
                        case 4:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.TakenSuccess",
                          });
                          statusColor = "text-green-500";
                          break;
                        case 5:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.TakenFail",
                          });
                          statusColor = "text-red-500";
                          break;
                        case 6:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.Delivering",
                          });
                          statusColor = "text-black";
                          break;
                        case 7:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.DeliveredCancel",
                          });
                          statusColor = "text-red-500";
                          break;
                        case 8:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.DeliveredFail",
                          });
                          statusColor = "text-red-500";
                          break;
                        case 9:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.Refunding",
                          });
                          statusColor = "text-black";
                          break;
                        case 10:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.RefundedSuccess",
                          });
                          statusColor = "text-green-500";
                          break;
                        case 11:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.RefundedFail",
                          });
                          statusColor = "text-red-500";
                          break;
                        case 12:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.EnterAgency",
                          });
                          statusColor = "text-black";
                          break;
                        case 13:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.LeaveAgency",
                          });
                          statusColor = "text-black";
                          break;
                        case 14:
                          statusLabel = intl.formatMessage({
                            id: "Consignment.Status.ThirdPartyDelivery",
                          });
                          statusColor = "text-black";
                          break;
                        default:
                      ...
                      return (
                        <span
                          className={`${statusColor} font-semibold text-xl`}
                        >
                          {statusLabel}
                        </span>
                      );
          ...
          >
            <>
              ...
              <span className="truncate">
                <FormattedMessage id="Consignment.Info.Submit2" />
              </span>
            </>
          </Button>
        </div>
